...
class ModisMap(object):
    ...
    def __create_edges(self, start, end, ratio):
        ...
        edges = []
        for i in range(i_search_range[0], i_search_range[1]):   # right side +1 ?
            for j in range(j_search_range[0], j_search_range[1]):
                for index in range(0, 8):
                    p0 = (i, j)
                    p1 = (i+offset[index][0], j+offset[index][1])
                    if self.prob_mat[p0[0], p0[1], 0] != np.nan and self.prob_mat[p1[0], p1[1], 0] != np.nan:
                        p0_index = self.__matrixcoor2index(p0[0], p0[1])
                        p1_index = self.__matrixcoor2index(p1[0], p1[1])
                        cost = 0.01 + ratio*(self.prob_mat[p1[0], p1[1], 1] + self.prob_mat[p1[0], p1[1], 2]) + (1-ratio)*dist[index]
